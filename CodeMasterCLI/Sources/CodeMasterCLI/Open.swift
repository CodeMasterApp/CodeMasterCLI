...
extension CodeMasterCLI {
    struct Open: ParsableCommand {
        static var configuration = CommandConfiguration(
            commandName: "open",
            abstract: "A command-line tool to open files/folders in CodeMaster.app."
        )
        ...
        func run() throws {
            ...
            if let path {
                ...
                if let line, !openURL.hasDirectoryPath {
                    task.arguments = ["-u", "codemaster://\(openURL.path):\(line):\(column ?? 1)"]
                } else {
                    task.arguments = ["-u", "codemaster://\(openURL.path)"]
                }
            } else {
                task.arguments = ["-a", "CodeMaster.app"]
            }
